...
export default function Navbar() {
	...
	return (
		<div ref={navbarRef}>
			...
			>
				<div
					className={`bg-white ${styles.pageContainer} flex items-center justify-between`}
				>
						...
						{menuBar.map(item => {
							return (
								<div key={item.id}>
									<Link
										to={item.url}
										className='flex delay-75 duration-150 ease-in-out fill-black hover:fill-hoverOrange stroke-black hover:stroke-hoverOrange flex-col gap-5 text-black hover:text-hoverOrange items-start justify-start'
									>
										<span>{item.logo}</span>
									...
									<p className='text-[13px] leading-[25px] font-opensans text-grayColor'>
										{item.num} модели
									</p>
								</div>
							)
